...
### Kategori Jadwal, Pengumuman, Guest Manual, Batas Tanggal (LIVE 26 Agu 2026)

- **Kategori sesi Umum/Private** (`SessionCategory` di ClassTemplate & ClassSession, migrasi `b6c7d8e9f0a1`): form Tambah Sesi & Template ada dropdown kategori; generate inherit. **Member (non-mine) hanya lihat sesi `umum`** (`list_sessions` filter) — private disembunyikan dari aplikasi member (info via WA). Staf lihat semua + badge "Private". **Broadcast WA** tampilkan semua sesi; sesi private berlabel **🔒 (Private Group)** tanpa info slot. Daftar sesi di pesan WA pakai **penomoran (1. 2. 3.)** bukan bullet.
- **Pengumuman member** (`StudioSettings.announcement` + `announcement_active`, migrasi `c7d8e9f0a1b2`): banner di dashboard member (via `GET /public/studio` yg kembalikan announcement bila aktif). Kelola di **Pengaturan → Pengumuman Member** (teks + toggle).
- **Tambah nama MANUAL ke sesi** (`POST /schedule/sessions/{id}/add-guest {name}`): buat User `per_datang` (email placeholder `tamu-xxxx@`, tanpa phone, password acak) + Booking BOOKED tanpa cek kuota. FE: input "Tambah nama manual" di RosterModal. Utk private class / walk-in; tercatat di tab Per Datang.
- **Batas bawah tanggal jadwal member** (`StudioSettings.member_schedule_start`, migrasi `d8e9f0a1b2c3`): bila di-set, member hanya lihat sesi `session_date >= start`. Setelan trial = **2026-08-28**. Kelola di Pengaturan (kosongkan = semua). Staf tak terpengaruh.
- **Import No. WA**: kini disimpan **format lokal ber-0** (`0812…`) walau Excel tanpa 0 (Excel drop leading-0 saat angka) — `member_import` normalisasi ke 62 lalu simpan 0-format.
- **★ Reset password member (trial)**: SEMUA 325 member di-reset ke `123456` (26 Agu, produksi) agar mudah login saat trial; owner/admin/instruktur TIDAK. Backup pra-reset `pilates_prepwd_*.sql.gz`. Pengumuman memuat info login (No. HP + 123456). ★ Setelah trial, imbau ganti password.
...
- **★ CRON DIPASANG DI VPS BARU (26 Agu):** setelah migrasi, cron broadcast/reminder **belum ada di VPS baru** → broadcast 20:00 tak jalan. Sudah dipasang di crontab produksi (6 entri: h1/h2/bulanan/dropin/expiry/expiry7, sama seperti VPS lama). ★ Logout device berlebih (`GET /app/logout` + header `X-Device-Id`) → **gateway kini SATU nomor: 6287837763692** (087837763692, anggota grup "Reformer Yourbody Coach ADE" `120363423874049546@g.us`). Grup tersimpan = grup itu. Broadcast H-2 diuji terkirim manual (`send_broadcasts --kind bulanan --force`, 6 sesi 28 Agu).
