...
- **Diskon perpanjangan** (`Package.renewal_discount` Rp, migrasi `e3f4a5b6c7d8`): potongan flat saat **admin** jual paket ke member yang **masih pegang paket SAMA (package_id) yang belum expired** (helper `services/purchase.eligible_renewal_discount`). Diterapkan otomatis di `sell_package` **bila harga tak di-override manual** (admin ketik harga = pakai apa adanya, tanpa diskon). `GET /members/{id}/purchase-quote?package_id` = pratinjau {base_price, renewal_discount, eligible, total}. **Hanya jalur admin** (bukan beli-sendiri member). FE: field **"Diskon perpanjangan"** di katalog Paket + kotak rincian harga (normal/diskon/total) di modal Jual Paket. ★ Setelan sekarang: GOLD 1 & GOLD 2 = Rp50.000, lainnya 0.
