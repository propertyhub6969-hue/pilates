...
async def main(kind: str, force: bool) -> None:
    ...
    label = {"h1": "H-1 (besok)", "expiry": "H-1 kedaluwarsa paket", "expiry7": "H-7 kedaluwarsa paket panjang"}.get(kind, f"H-{settings.REMINDER_HOURS_BEFORE}jam (mulai sebentar lagi)")
    print(f"== Reminder WA [{label}] — mode: {mode} ==")
    async with AsyncSessionLocal() as db:
        if kind == "expiry":
            res = await run_expiry_reminders(db, days_before=1, force=force)
        elif kind == "expiry7":
            res = await run_expiry_reminders(db, days_before=7, force=force)
        else:
            res = await run_reminder_pass(db, kind=kind, force=force)
        await db.commit()
...
if __name__ == "__main__":
    ...
    p.add_argument("--kind", choices=["h1", "h2", "expiry", "expiry7"], default="h1")
